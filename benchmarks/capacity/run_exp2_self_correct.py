#!/usr/bin/env python3
"""
Experiment 2: Error Feedback & 1-Chance Self-Correction Loop for tau-bench (Dual-Node Config).

Intercepts JSON parsing errors (JSONDecodeError) or protocol validation errors,
constructs a feedback turn containing the exact error message, and gives the model
1 immediate retry turn to self-correct its output across 2 dedicated vLLM inference nodes.
"""
...
class SelfCorrectingLLMAgent(LLMAgent):
    """LLM Agent that feeds JSON parsing/validation errors back to the model for 1 self-correction turn."""
    ...
    def _generate_next_message(
        ...
        try:
            ...
            error_feedback = (
                f"ERROR: Your previous response failed validation with error: {error}.\n"
                f"Please fix your response immediately. Output ONLY a valid JSON format matching the schema."
            )
...
def create_self_correcting_agent(tools, domain_policy, **kwargs):
    return SelfCorrectingLLMAgent(
        tools=tools,
...
registry.register_agent_factory(create_self_correcting_agent, "self_correcting_agent")


def main():
    parser = argparse.ArgumentParser(description="Experiment 2: Error Feedback Self-Correction Runner (Dual-Node)")
    ...
    parser.add_argument("--output-dir", default="benchmarks/capacity/results/exp2_self_correct")
    ...
    output_json = os.path.join(args.output_dir, "exp2_self_correct_results.json")
    ...
    asyncio.run(
        run_tau_capacity_suite(
            ...
            num_trials=1,
            agent_name="self_correcting_agent",
            max_steps=30,
